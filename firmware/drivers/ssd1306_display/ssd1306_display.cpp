...
#error "Only 32, 64, or 128 lines of height are supported!"
  ...
  WriteCommand(0x14); //
  SetDisplayOn(1);    //--turn on SSD1306 panel
...
void Fill(SSD1306_COLOR color) { memset(SSD1306_Buffer, (color == Black) ? 0x00 : 0xFF, sizeof(SSD1306_Buffer)); }
...
static float DegToRad(float par_deg) { return par_deg * (3.14f / 180.0f); }

/* Normalize degree to [0;360] */
static uint16_t NormalizeTo0_360(uint16_t par_deg) {
  uint16_t loc_angle;
  if (par_deg <= 360) {
    loc_angle = par_deg;
  } else {
...
void DrawArcWithRadiusLine(uint8_t x, uint8_t y, uint8_t radius, uint16_t start_angle, uint16_t sweep, SSD1306_COLOR color) {
  ...
  // Radius line
  Line(x, y, first_point_x, first_point_y, color);
  Line(x, y, xp2, yp2, color);
  return;
}

/* Draw circle by Bresenhem's algorithm */
void DrawCircle(uint8_t par_x, uint8_t par_y, uint8_t par_r, SSD1306_COLOR par_color) {
  int32_t x = -par_r;
  int32_t y = 0;
...
uint8_t GetDisplayOn() { return SSD1306.DisplayOn; }
